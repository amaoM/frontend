...
describe('component::IncidentSearchForm', () => {
  ...
  const component = shallow(
    <IncidentSearchForm {...incidentSearchFormProps} />,
    { context: store }
  );
  describe('when IncidentSearchForm is renderd', () => {
    it('should be rendered keyword, level and status fields with setted values', () => {
      ...
      expect(component.find('input[name="keyword"]').prop('value')).to.equal(
        'test'
      );
      expect(
        component
          .find('select[name="level"] option[defaultValue=true]')
          .prop('value')
      ).to.equal('critical');
      expect(
        component
          .find('select[name="status"] option[defaultValue=true]')
          .prop('value')
